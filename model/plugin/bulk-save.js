...
module.exports = function bulkSaveSchemaPlugin(schema, options) {
	...
	schema.method('bulkSave', function bulkSave(options) {
		...
		return doc.validate().then(() => {
				...
				console.verbose(`[model ${model.modelName}].bulkSave model._bulkSaveDeferred=${model._bulkSaveDeferred?inspect(model._bulkSaveDeferred):'(undefined)'}`);	// action=${actionType}
				// if (actionType === 'check') {
				...
				function innerBulkSave(bs, bsd) {
					...
					model.bulkWrite(bulkOps).then(bulkWriteOpResult => {	//bsEntry.op)).then(bulkWriteOpResult => {
						...
						if (r.ok) {
							model._stats.bulkSave.success += r.nModified + r.nInserted + r.nUpserted + r.nMatched;

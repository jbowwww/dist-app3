	...
	*/
	...
	logStats() {
		console.verbose( `mongoose.models count=${_.keys(mongoose.models).length} names=${mongoose.modelNames().join(', ')}\n` + 
			...
			`Tasks.all (${Task.all.length}): ${inspect(Task.all, { depth: 3, compact: true } )} Tasks.uniqueContexts (${Task.uniqueContexts.length})=${inspect(Task.uniqueContexts, { depth: 3, compact: true })}`);
		app.logErrors();
	},
	logErrors() {

...
function chainPromiseFuncs(chain) {
 ...
 */
function promisePipe(sourceStream, promiseFunctions, options = {}) {
	...
	if (_.isArray(promiseFunctions) && _.every(promiseFunctions, element => _.isFunction(element))) {
		promiseFunctions = chainPromiseFuncs(promiseFunctions);
	}
		...
		sourceStream.pipe(
			new stream.Writable({
				...
				write(data, encoding, callback) {
					...
					.catch(err => { options.enableStreamErrors ? callback(err) : callback(); })
					.done();
				}
			})
		), 'finish'
...
promisePipe.conditional = function promisePipe_conditional(condition, pipe1, pipe2 = null) {
	return (data => condition(data) ? pipe1(data) : (pipe2 ? pipe2(data) : data));
};

promisePipe.chainFunctions = chainPromiseFuncs;
